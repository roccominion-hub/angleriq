...
export async function sendWelcomeEmail(to: string, name: string) {
  const firstName = name?.split(' ')[0] || 'Angler'
  const html = baseTemplate(`
    ...
    <p>
      ...
      <strong>3. Expand the filters</strong> — dial in by bait type, structure, depth, and more.
    </p>
...
export async function sendUpgradeEmail(to: string, name: string) {
  ...
  const html = baseTemplate(`
    ...
    <p>We're constantly adding new lakes and tournament data, so check back often — the intel gets better every week.</p>
  ...
  return resend.emails.send({
    from: FROM,
    to,
    subject: "You're a Pro — AnglerIQ subscription confirmed ✅",
    html,
  })
}
